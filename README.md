...
- 기능별 서버 분리: 입고, 출고, 재고(위치/이동), QR 스캔 기능이 각각 별도의 독립된 방(컨테이너)에서 실행됩니다. 하나의 기능에 문제가 생기거나 사용자가 몰려도 다른 기능은 영향을 받지 않는다.

- 데이터 저장 방식: 중요한 마스터 정보는 메인 저장소(PostgreSQL)에 안전하게 저장하고, 자주 쓰는 정보는 초고속 임시 저장소(Redis)에 두어 화면이 빠르게 뜨도록 합니다. 현장에서 QR 코드를 대량으로 찍을 때 발생하는 과부하는 중간 완충 장치(Kafka)가 받아주어 서버가 다운되는 것을 막는다.
<br>
...
Docker, Kubernetes, Harbor 등의 구성 작업을 자동 실행한다다.
DB는 사전에 구성해서 제외한다.
